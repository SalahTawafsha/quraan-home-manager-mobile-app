        ...
        database.child("teachers").child(name.getText().toString().trim()).get().addOnCompleteListener(task -> {
            ...
            if (t != null) {
                ...
                if (this.password.getText().toString().equals(password)) {
                    ...
                    startActivity(intent);
                } else Toast.makeText(this, "كلمة السر خاطئة", Toast.LENGTH_SHORT).show();
            } else {
                database.child("assistants").child(name.getText().toString().trim()).get().addOnCompleteListener(task1 -> {
                    ...
                    if (a != null) {
                        ...
                        if (this.password.getText().toString().equals(password)) {
                            if (!a.getTeacherName().isEmpty()) {
                                ...
                                Toast.makeText(this, "لا يمكنك تسجيل الدخول.\nلست مسجلاً لدى اي مدرس", Toast.LENGTH_SHORT).show();
                        } else Toast.makeText(this, "كلمة السر خاطئة", Toast.LENGTH_SHORT).show();
                    } else Toast.makeText(this, "اسم المعلم غير موجود", Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
